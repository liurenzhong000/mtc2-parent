...
APP_JAR_NAME=service-notification
APP_JAR_PATH=./$APP_JAR_NAME.jar
MEM=512

case $1 in
    start)
    nohup java -server -Xms${MEM}m -Xmx${MEM}m -XX:-UseGCOverheadLimit -XX:NewRatio=1 -XX:SurvivorRatio=8 -XX:+UseSerialGC -jar $APP_JAR_PATH --spring.profiles.active=test  >/dev/null 2>&1 &
